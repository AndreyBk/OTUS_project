...
class htswitcher_stability extends Simulation {
  // точка входа
...
//        // сценарий нагрузки hts
        Scenario_load_hts_analogue().inject(
          rampConcurrentUsers(0).to(40).during(60 seconds),
          constantConcurrentUsers(40) during (1 hour),
//          rampConcurrentUsers(40).to(0).during(60 seconds)
